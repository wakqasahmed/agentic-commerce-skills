...
jq -e '
  def substantive:
    ...
    ((ascii_downcase | sub("[.!]$"; "")) as $value |
      ["tbd", "todo", "pending", "unknown", "n/a", "na", "none", "later"] |
      index($value) | not);
  ...
  all(.[];
    if (.operation_mode == "action_capable" or .risk_level == "high") then
      ...
      (.operational_controls.safe_dependency_fallback | checkable("\\b(defer|manual|preserve|queue|read-only|route|stop)\\w*\\b.*\\b(checkout|customer|intent|operator|request|support|write)\\w*\\b"))
    else true end
  )' "$PLAN"
```
